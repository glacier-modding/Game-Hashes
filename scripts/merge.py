...
import os
import argparse
from common import GAME_FLAGS_BOND, COMBINED_FLAGS, PlatformTag, ioi_hash
...
parser.add_argument('-g', '--game', choices=COMBINED_FLAGS.keys(), nargs='*', help="Game to generate the hash list for. Defaults to all.")
parser.add_argument('-o', '--output', type=str, default="hash_list.txt", help="Output file name. Defaults to hash_list.txt.")
args = parser.parse_args()

if not args.game:
    args.game = list(COMBINED_FLAGS.keys())
...
for repo in repositories:
    input_directory = os.path.join(repo, "paths")
    ...
    for json_file in json_files:
        ...
        resource_type = os.path.splitext(os.path.basename(json_file))[0]

        for entry in data:
            if "Bond-Hashes" in repo:
                if entry["gameFlags"] & GAME_FLAGS_BOND["FirstLight"]:
                    entry["gameFlags"] = COMBINED_FLAGS["FirstLight"]
            if entry.get('gameFlags') is None or not any(entry['gameFlags'] & COMBINED_FLAGS[game] for game in args.game):
                continue
            if entry.get('path'):
                path = entry['path']
            elif entry.get('hint'):
            ...
            total_hashes += 1
            if entry["hash"].startswith("01"):
                calculated_hash = ioi_hash(path, PlatformTag.Pc)
            else:
